...
ytt -f tap-values-run.yaml -f values.yaml -f values-test.yaml --ignore-unknown-comments > generated/tap-values-test.yaml


tanzu package install tap -p tap.tanzu.vmware.com -v $1 --values-file generated/tap-values-test.yaml -n tap-install

# install external dns
kubectl create ns tanzu-system-ingress
ytt --ignore-unknown-comments -f values.yaml -f values-test.yaml -f ingress-config/ | kubectl apply -f-
